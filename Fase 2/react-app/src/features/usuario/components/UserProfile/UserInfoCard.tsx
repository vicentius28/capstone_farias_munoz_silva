...
import UserContactosEmergencia from "../profile/UserContactosEmergencia";
...
const UserInfoCard: React.FC<Props> = ({ user, image, formatDate, userId }) => {
  const location = useLocation();
  const [imageLoading, setImageLoading] = useState(true);
  const [imageError, setImageError] = useState(false);

  const isProfilePage = location.pathname === "/perfil";
  const isUserProfilePage = location.pathname === "/ficha/user-profile";
  const shouldShowEmergencyContacts = isProfilePage || isUserProfilePage;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-4">
      ...
      {/* Contactos de Emergencia - En perfil y ficha/user-profile */}
      {shouldShowEmergencyContacts && (
        <Card
          className="bg-white dark:bg-gray-900 border border-red-200 dark:border-red-800 shadow-sm"
          radius="lg"
        >
          <CardBody className="p-4 sm:p-6">
            <UserContactosEmergencia userId={userId} />
          </CardBody>
        </Card>
      )}
    </div>
