...
from proyectoapp.models import (
    User, Evaluacion, UsuarioTitulo, UsuarioMagister, UsuarioDiplomado,
    Bienios, Tramos, Cargo, Ciclo, Codigo, Genero,Capacitacion,ParticipacionCapacitacion,ContactoEmergencia,TituloCapacitacion
)
...
class UserResource(resources.ModelResource):
    ...
    def dehydrate_titulos(self, user):
        return ", ".join(
            [str(t.titulo) for t in UsuarioTitulo.objects.filter(usuario=user)]
        )

    def dehydrate_magister(self, user):
        return ", ".join(
            [str(m.magister) for m in UsuarioMagister.objects.filter(usuario=user)]
        )

    def dehydrate_diplomados(self, user):
        return ", ".join(
            [str(d.diplomado) for d in UsuarioDiplomado.objects.filter(usuario=user)]
        )

    def dehydrate_bienios(self, user):
        return ", ".join(
            [f"{b.bienios} ({b.tramo})" for b in Bienios.objects.filter(usuario=user)]
        )
...
class UsuarioTituloInline(admin.TabularInline):
    model = UsuarioTitulo
    extra = 0

class UsuarioMagisterInline(admin.TabularInline):
    model = UsuarioMagister
    extra = 0

class UsuarioDiplomadoInline(admin.TabularInline):
    model = UsuarioDiplomado
    extra = 0

class UsuarioBieniosInline(admin.TabularInline):
    model = Bienios
    extra = 0
...
class CustomUserAdmin(HistorialAdminMixin):
    ...
    inlines = [UsuarioTituloInline, UsuarioMagisterInline, UsuarioDiplomadoInline, UsuarioBieniosInline, AsignacionTrabajadorInline]
...
admin.site.register(Tramos, TramosAdmin)
admin.site.register(Bienios, BieniosAdmin)
admin.site.register(Cargo, CargoAdmin)
admin.site.register(Ciclo, CicloAdmin)
admin.site.register(Codigo, CodigoAdmin)
admin.site.register(Genero, GeneroAdmin)

admin.site.register(TituloCapacitacion,TituloCapacitacionAdmin)
admin.site.register(Capacitacion, CapacitacionAdmin)
admin.site.register(ParticipacionCapacitacion, ParticipacionCapaAdmin)
admin.site.register(ContactoEmergencia, ContactoEmergenciaAdmin)
@admin.register(Evaluacion)
class EvaluacionAdmin(admin.ModelAdmin):
    list_display = ("usuario", "porcentaje", "anio", "drive_url", "fecha_creacion")
    search_fields = ("usuario__username", "anio")
    list_filter = ("anio",)
