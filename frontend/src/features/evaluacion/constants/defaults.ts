...
export const DEFAULT_NIVELES = [
  {
    nombre: "DESTACADO 4 puntos",
    nvl: 4,
    puntaje: 4,
    descripcion: "Siempre (4 OA).",
  },
  {
    nombre: "COMPETENTE 3 puntos",
    nvl: 3,
    puntaje: 3,
    descripcion: "Constantemente (3 OA).",
  },
  {
    nombre: "INICIAL 2 puntos",
    nvl: 2,
    puntaje: 2,
    descripcion: "Ocasionalmente (2 OA).",
  },
  {
    nombre: "INSATISFACTORIO 1 punto",
    nvl: 1,
    puntaje: 1,
    descripcion: "Rara vez o nunca (1 o ninguna OA).",
  },
];
...
export const EVALUATION_CRITERIA = {
  ...
  RANGES: {
    DESTACADO: { min: 90, max: 100 },
    COMPETENTE: { min: 80, max: 89 },
    INICIAL: { min: 70, max: 79 },
    INSATISFACTORIO: { min: 0, max: 69 },
  },
...
export type EvaluationColor =
  (typeof EVALUATION_CRITERIA.COLORS)[EvaluationLevel];
...
export const EvaluationUtils = {
  ...
  getLevel: (percentage: number): EvaluationLevel => {
    if (percentage >= EVALUATION_CRITERIA.RANGES.DESTACADO.min)
      return "destacado";
    if (percentage >= EVALUATION_CRITERIA.RANGES.COMPETENTE.min)
      return "competente";
  ...
  isInRange: (percentage: number, targetLevel: EvaluationLevel): boolean => {
    const range =
      EVALUATION_CRITERIA.RANGES[
        targetLevel.toUpperCase() as keyof typeof EVALUATION_CRITERIA.RANGES
      ];
