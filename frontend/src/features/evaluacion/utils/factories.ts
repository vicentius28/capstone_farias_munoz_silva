...
type IndicadorOverrides = Partial<
  Pick<
    Indicador,
    "indicador" | "definicion" | "nvlindicadores" | "numero" | "id"
  >
>;

/** Crea un indicador con valores por defecto y permite overrides */
export const makeIndicador = (
  numero: number,
  baseId?: number,
  ...
  nvlindicadores: (overrides.nvlindicadores?.length
    ? overrides.nvlindicadores
    : cloneNiveles()) as NivelLogro[],
});

/** Crea N indicadores, permitiendo generar a partir de overrides por cada índice */
export const makeIndicadores = (
  cantidad: number,
  baseId?: number,
  items?: Array<IndicadorOverrides>,
): Indicador[] => {
...
export const makeCompetencia = (
  id: number,
  name = "Competencia",
  indicadores: Indicador[] | number = 1,
): Competencia => {
