...
import {
  ...
  DetalleAsignacion,
} from "@/features/evaluacion/types/asignar/evaluacion";

export type DetalleConAsignacion = DetalleAsignacion & {
  asignacion: AsignacionEvaluacion;
};

export function flattenDetalles(
  asignaciones: AsignacionEvaluacion[],
): DetalleConAsignacion[] {
...
export function filtrarDetalles(
  list: DetalleConAsignacion[],
  ...
  if (filtros.año) {
    list = list.filter(
      (item) =>
        new Date(item.asignacion.fecha_evaluacion).getFullYear().toString() ===
        filtros.año,
    );
  }
