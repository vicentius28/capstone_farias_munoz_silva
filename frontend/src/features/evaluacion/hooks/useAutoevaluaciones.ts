  ...
  const fetchData = useCallback(async (signal: AbortSignal) => {
    try {
      ...
      const [p, f] = await Promise.all([
        axios.get("/evaluacion/api/autoevaluaciones/?completado=false", {
          signal,
        }),
        axios.get("/evaluacion/api/autoevaluaciones/?completado=true", {
          signal,
        }),
      ]);
      ...
      addToast({
        ...
        description:
          "Ocurrió un problema al intentar cargar las autoevaluaciones.",
        color: "danger",
        variant: "solid",
      });
    } finally {
