...
export async function marcarReunionRealizada(
  ...
  try {
    ...
    return {
      success: false,
      message:
        error.response?.data?.error || "Error al marcar reunión como realizada",
    };
  }
}
...
export async function completarRetroalimentacion(
  ...
  try {
    ...
    return {
      success: false,
      message:
        error.response?.data?.error || "Error al completar retroalimentación",
    };
  }
}
...
export async function completarRetroalimentacionYCerrar(
  ...
  try {
    ...
    const responseRetro = await completarRetroalimentacion(
      evaluacionId,
      retroalimentacion,
    );
    ...
    if (responseFirma.success) {
      return {
        success: true,
        message:
          "Retroalimentación completada y evaluación cerrada para firma exitosamente",
...
export async function obtenerMisEvaluacionesJefatura(): Promise<
  EvaluacionJefe[]
> {
  try {
...
export async function descargarPDFEvaluacionJefe(
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.error ||
        "Error al descargar el PDF de evaluación de jefatura",
    );
  }
}
...
export async function descargarPDFAutoevaluacion(
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.error ||
        "Error al descargar el PDF de autoevaluación",
    );
  }
}
...
export async function descargarPDFMisEvaluaciones(
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.error ||
        "Error al descargar el PDF de mi evaluación",
    );
  }
}
