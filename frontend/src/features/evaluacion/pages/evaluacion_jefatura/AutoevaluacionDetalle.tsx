...
export default function AutoevaluacionDetallePage() {
  ...
  const [autoevaluacion, setAutoevaluacion] = useState<Autoevaluacion | null>(
    null,
  );
  ...
  const backUrl =
    ...
    "/evaluacion-jefatura/autoevaluaciones-subordinados";

  // Priorizar estructura_json (snapshot) sobre tipo_evaluacion
  const estructura =
    autoevaluacion?.estructura_json ?? autoevaluacion?.tipo_evaluacion;
    ...
    (async () => {
      ...
      try {
        const response = await axios.get(
          `/evaluacion/api/autoevaluaciones-subordinados/${id}/`,
        );
  ...
  const respuestasMap = useMemo(() => {
    ...
    return autoevaluacion.respuestas.reduce(
              ...
              const indicadoresCalculados = (comp?.indicadores ?? []).map(
                (ind: any) => {
                  ...
                  return {
                    ...
                    porcentaje:
                      maxPuntaje > 0 ? (puntajeResp / maxPuntaje) * 100 : 0,
                  } as IndicadorDetalle;
                },
              );
