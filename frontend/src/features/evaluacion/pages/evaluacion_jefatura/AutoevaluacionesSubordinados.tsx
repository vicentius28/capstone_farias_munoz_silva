...
const getYearFromItem = (item: AutoevaluacionSubordinado): string => {
  const dateStr =
    item.fecha_periodo || item.fecha_inicio || item.fecha_ultima_modificacion;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (isMounted) {
          const grupos: GrupoAutoevaluacionesData[] = Array.isArray(
            response.data,
          )
            ? response.data
            : [];
...
export default function TeamReviewsDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"pendientes" | "finalizadas">(
    "pendientes",
  );
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      if (searchTerm) {
        ...
        const fullName =
          `${item.persona.first_name} ${item.persona.last_name}`.toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#09090b] font-sans text-slate-600 relative">
      <div
        className="absolute inset-0 z-0 pointer-events-none text-slate-400/20"
        style={{
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 space-y-8 relative z-10">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Supervisa el progreso de las autoevaluaciones de tu equipo
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            bgIcon="bg-violet-50"
            icon={<ChartPieIcon className="w-5 h-5 text-violet-600" />}
          ...
          <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800 gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
              <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-lg shrink-0">
                <button
                  className={`flex-1 sm:flex-none px-4 py-2 text-sm font-semibold rounded-md transition-all ${activeTab === "pendientes" ? "bg-white dark:bg-slate-700 text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  onClick={() => setActiveTab("pendientes")}
                >
                  Pendientes
                </button>
                <button
                  className={`flex-1 sm:flex-none px-4 py-2 text-sm font-semibold rounded-md transition-all ${activeTab === "finalizadas" ? "bg-white dark:bg-slate-700 text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            ...
            <div className="flex items-center gap-3 w-full xl:w-auto justify-between sm:justify-end overflow-x-auto pb-1 sm:pb-0">
              ...
              <Dropdown>
                <DropdownTrigger>
                  <Button
                    className="bg-slate-50 dark:bg-slate-800 text-slate-600 min-w-fit"
                    variant="flat"
                  >
              ...
              <Dropdown>
                <DropdownTrigger>
                  <Button
                    className="border-slate-200 text-slate-600 capitalize min-w-fit"
                    variant="bordered"
                  >
                ...
                <DropdownMenu
                  ...
                  selectionMode="single"
                  onAction={(key) => setSelectedYear(key as string)}
                >
              ...
              <div className="flex items-center bg-slate-50 dark:bg-slate-800 rounded-lg p-1 border border-slate-200 dark:border-slate-700 shrink-0">
                <button
                  className={`p-1.5 rounded-md transition-all ${viewMode === "grid" ? "bg-white dark:bg-slate-600 text-blue-600 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}
                  onClick={() => setViewMode("grid")}
                >
                  <Square2StackIcon className="w-5 h-5" />
                </button>
                <button
                  className={`p-1.5 rounded-md transition-all ${viewMode === "list" ? "bg-white dark:bg-slate-600 text-blue-600 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}
                  ...
                  >
                    {filteredData.map((item) => (
                      <ReviewItem
                        ...
                        onClick={() =>
                          navigate(
                            "/evaluacion-jefatura/autoevaluacion-detalle",
                            { state: { id: item.id, autoevaluacion: item } },
                          )
                        }
                      />
                    ))}
                      ...
                      Object.entries(groupedData).map(([groupTitle, items]) => (
                        <div key={groupTitle} className="space-y-4">
                          ...
                          <div className="flex items-center gap-3 sticky top-0 bg-slate-50/95 dark:bg-slate-900/95 py-2 z-10 backdrop-blur-sm">
                            <div
                              className={`p-1.5 rounded-lg ${groupBy === "period" ? "bg-blue-100 text-blue-600" : "bg-purple-100 text-purple-600"}`}
                            >
                              {groupBy === "period" ? (
                          ...
                          <div
                            className={
                              viewMode === "grid"
                                ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pl-4"
                                : "space-y-3 pl-4"
                            }
                          >
                            {items.map((item) => (
...
function ReviewItem({
  ...
  item: AutoevaluacionSubordinado;
  viewMode: "grid" | "list";
  onClick: () => void;
}) {
  if (viewMode === "grid")
    return <ReviewGridCard data={item} onClick={onClick} />;
...
function StatCard({ title, value, sublabel, icon, bgIcon, highlight }: any) {
  return (
    <Card
      className={`border shadow-sm bg-white dark:bg-slate-900 ${highlight ? "border-amber-200 bg-amber-50/50" : "border-slate-200 dark:border-slate-800"}`}
    >
      <CardBody className="p-5">
        <div className="flex flex-col h-full justify-between">
          <span
            className={`text-sm font-semibold mb-2 ${highlight ? "text-amber-700" : "text-slate-500"}`}
          >
            {title}
          </span>
          <div className="flex items-end justify-between">
            <div>
              <span
                className={`text-3xl font-bold block ${highlight ? "text-amber-900" : "text-slate-900 dark:text-white"}`}
              >
                {value}
              </span>
              {sublabel && (
                <span
                  className={`text-xs font-medium mt-1 block ${highlight ? "text-amber-600" : "text-slate-400"}`}
                >
                  {sublabel}
                </span>
              )}
...
function ReviewGridCard({
  ...
  data: AutoevaluacionSubordinado;
  onClick: () => void;
}) {
    ...
    >
      <div>
        <div className="flex items-start justify-between mb-4">
          <div
            className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shadow-sm ${data.completado ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-amber-50 text-amber-600 border border-amber-100"}`}
          >
            {initials}
          </div>
          {data.completado ? (
            <div className="px-2 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase rounded-md border border-emerald-100">
              Finalizada
            </div>
          ) : (
            <div className="px-2 py-1 bg-amber-50 text-amber-700 text-[10px] font-bold uppercase rounded-md border border-amber-100 animate-pulse">
              Pendiente
            </div>
          )}
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div
          className={`w-10 h-10 flex-shrink-0 rounded-full flex items-center justify-center text-xs font-bold ${data.completado ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
        >
          {initials}
        </div>
      ...
      <div className="hidden sm:flex flex-col items-end mr-6 min-w-[120px]">
        <span
          className={`text-xs font-bold uppercase mb-1 ${data.completado ? "text-emerald-600" : "text-amber-600"}`}
        >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
      ...
      <div className="flex gap-2 mt-4">
        ...
        {yearFiltered && (
          <Button color="primary" size="sm" variant="light" onClick={clearYear}>
            Ver todos los años
          </Button>
        )}
      </div>
