...
const isWeightedEvaluation = (
  data?: JefaturaEvaluacionDetail | null,
): boolean => !!(data?.es_ponderada ?? data?.ponderada ?? false);

// ─────────────────────────────────────────────────────────────────────
// HOOKS PERSONALIZADOS
// ─────────────────────────────────────────────────────────────────────
const useProcessedAreas = (
  detailAreas?: AreaDetalleExtended[] | null,
): ProcessedArea[] => {
  return useMemo(() => {
    ...
    return detailAreas
      ...
      .map((area) => ({
        ...
        aporte:
          area.obtenido != null
            ? (area.obtenido * area.ponderacion) / 100
            : null,
      }));
  }, [detailAreas]);
};
...
const useWeightedStats = (areas: ProcessedArea[]) => {
  return useMemo(() => {
    ...
    const totalContribution = areas.reduce(
      (acc, area) => acc + (area.aporte ?? 0),
      0,
    );
...
const useAreasForSummary = (
  estructura: any,
  respuestasMap: Record<number, number>,
): AreaDetalle[] => {
  return useMemo(() => {
...
// ─────────────────────────────────────────────────────────────────────
const WeightedAreasTable = ({
  areas,
  stats,
}: {
  areas: ProcessedArea[];
  stats: ReturnType<typeof useWeightedStats>;
}) => (
  <Card className="mb-6 shadow-lg">
    <CardHeader>
      <div className="flex items-center justify-between w-full">
        <h3 className="font-semibold">
          Desglose por Áreas (Evaluación Ponderada)
        </h3>
    ...
    <CardBody>
            ...
            {areas.map((area, index) => (
              ...
              >
                ...
                <td className="py-3 pr-4">
                  {area.obtenido != null ? (
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${getBadgeStyle(area.obtenido).soft}`}
                    >
                      {area.obtenido}%
                    </span>
                  ) : (
      ...
      <div className="mt-4 rounded-lg bg-default-50 p-3">
        <p className="text-xs text-default-600">
          <strong>Contribución total calculada:</strong>{" "}
          {Math.round(stats.totalContribution)} puntos porcentuales
        </p>
        <p className="text-xs text-default-500 mt-1">
          * El aporte indica cuántos puntos suma cada área al resultado final
      ...
      <Card className="w-full max-w-md shadow-lg">
        <CardBody className="p-8">
          ...
          <p className="mt-2 text-sm text-default-600">
            No se pudo encontrar la evaluación solicitada. Vuelve al listado
...
export default function JefaturaEvaluacionDetallePage() {
  const { state } = useLocation() as {
    state?: { id?: number; firmado?: boolean };
  };
  ...
  const titulo =
    ...
    data?.tipo_evaluacion?.n_tipo_evaluacion ||
  ...
  const fetchEvaluationDetail = useCallback(async () => {
    ...
    try {
      ...
      setIsReadOnly(
        response.data.firmado || response.data.estado_firma === "firmado_obs",
      );
      ...
      addToast({
        ...
        description:
          "No se pudo cargar la evaluación. Intenta de nuevo en unos minutos.",
        color: "danger",
        variant: "solid",
      });
    } finally {
  ...
  const handleAcceptEvaluation = async () => {
    ...
    if (data.firmado) {
      addToast({
        title: "Evaluación ya aceptada",
        description: "Esta evaluación ya ha sido aceptada anteriormente.",
        color: "warning",
        variant: "solid",
      });
    ...
    if (!data.cerrado_para_firma) {
      addToast({
        title: "Evaluación no disponible para aceptación",
        description:
          "La evaluación debe estar cerrada para aceptación por la jefatura antes de poder aceptarla.",
        color: "warning",
        variant: "solid",
      });
    ...
    try {
      ...
      addToast({
        ...
        description: "Se ha marcado como aceptada exitosamente.",
        color: "success",
        variant: "solid",
      });
      ...
      const errorMessage =
        ...
        "No se pudo aceptar la evaluación. Revisa tu conexión e inténtalo otra vez.";
  ...
  const handleRejectEvaluation = async (reason: string) => {
    ...
    if (!data?.cerrado_para_firma) {
      addToast({
        ...
        description:
          "La evaluación debe estar cerrada para aceptación antes de poder rechazarla.",
        color: "warning",
        variant: "solid",
      });
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error ||
        "No se pudo denegar la evaluación. Revisa tu conexión e inténtalo otra vez.";
  ...
  const getEstadoEvaluacion = (): string | undefined => {
    ...
    if (data.estado_firma === "firmado_obs") {
      const fechaTexto = data.fecha_firma
        ? ` el ${formatearFecha(data.fecha_firma)}`
        : "";

      return `Proceso Completado (aceptado por el trabajador${fechaTexto})`;
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mt-8 space-y-6">
          ...
          <ActionButtons
            isReadOnly={
              isReadOnly ||
              !data?.cerrado_para_firma ||
              data?.estado_firma === "firmado_obs"
            }
            saving={saving}
