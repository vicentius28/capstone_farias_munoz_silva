...
export default function Pagination({
  ...
  const { isAreaCompleta } = useMemo(() => {
    ...
    const totalIndicadores = areaActual.competencias.reduce(
      (acc: number, comp: AreaEvaluacion["competencias"][0]) =>
        acc + comp.indicadores.length,
      0,
    );

    if (totalIndicadores === 0)
      return { progresoActual: 100, isAreaCompleta: true };
    ...
    areaActual.competencias.forEach((comp: any) => {
      comp.indicadores.forEach((ind: any) => {
        ...
        const tieneRespuesta = respuestas.some(
          (r) => r.indicador == ind.id && r.puntaje > 0,
        );
