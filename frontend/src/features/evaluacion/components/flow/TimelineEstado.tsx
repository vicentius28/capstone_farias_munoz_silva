...
function getEstadoSteps(evaluacion: EvaluacionJefe): EstadoStep[] {
  ...
  if (evaluacion.firmado || evaluacion.firmado_obs) {
    const tienObservaciones = !!(
      evaluacion.firmado_obs && evaluacion.motivo_denegacion
    );

    steps.push({
      id: "firmado",
      title: tienObservaciones
...
function getEstadoActual(evaluacion: EvaluacionJefe): {
  ...
  if (evaluacion.firmado || evaluacion.firmado_obs) {
    return {
      estado: "finalizado",
      descripcion: evaluacion.firmado_obs
        ? "Firmado con observaciones"
...
export function TimelineEstado({
  evaluacion,
  className = "",
}: TimelineEstadoProps) {
        ...
        <CardBody>
          <div className="text-center py-8">
            ...
            <p className="text-default-500">
              La evaluación aún no ha sido completada
            </p>
            <Chip className="mt-2" color="default" variant="flat">
