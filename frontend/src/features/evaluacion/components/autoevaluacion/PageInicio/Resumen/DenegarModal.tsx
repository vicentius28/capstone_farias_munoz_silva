...
export default function DenegarModal({
  open,
  onClose,
  onSubmit,
  loading,
  ...
  const isUnderMinimum =
    motivo.trim().length > 0 && motivo.trim().length < minLen;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-white/60 bg-white p-6 shadow-2xl">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Explica brevemente la observación de la evaluación y confirma que
          aceptas con observación.
        </p>
        ...
        <div className="mt-1 flex items-center justify-between text-xs">
          <div className="flex flex-col">
            <span className="text-gray-500">
              Mín. {minLen} - Máx. {maxLen} caracteres
            </span>
            {isUnderMinimum && (
        ...
        <label className="mt-3 flex items-start gap-2 text-sm">
          ...
          <span className="text-gray-700">
            Confirmo que acepto la evaluación con observación y que quedará
            registrada.
          </span>
        </label>
